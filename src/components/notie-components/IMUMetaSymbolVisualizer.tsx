...
function SceneContents({
    ...
    return (
        <>
            ...
            <directionalLight
                position={[6, 8, 7]}
                intensity={darkMode ? 0.9 : 1.15}
            />
            <directionalLight position={[-6, -3, 5]} intensity={0.35} />
            <gridHelper
                args={[14, 14, gridCenter, gridLines]}
                position={[0, 0, -1.1]}
            />
            <axesHelper args={[1.8]} />
            {truth3.length >= 2 && (
            ...
            {currentBodyAxes.map((axis, idx) => {
                ...
                return (
                    <group key={labels[idx]}>
                        <Line
                            points={toThree([truthCurrent, end])}
                            color={colors[idx]}
                            lineWidth={2.6}
                        />
...
function MetricCard({ title, value, subtitle, color }: MetricCardProps) {
    return (
        <Paper
            variant="outlined"
            sx={{ p: 1.5, borderRadius: 3, minWidth: 180, flex: 1 }}
        >
            <Typography
                variant="caption"
...
export default function IMUMetaSymbolVisualizer() {
    ...
    const [trajectoryMode, setTrajectoryMode] =
        useState<TrajectoryMode>("lissajous");
    ...
    useEffect(() => {
        const stepSimulation = (dt: number) => {
            ...
            const imuGyro =
                gyroNoiseSigma > 0
                    ? noisyVec(truth.gyroBody, gyroNoiseSigma)
                    : truth.gyroBody;
            const imuAccel =
                accelNoiseSigma > 0
                    ? noisyVec(truth.fBody, accelNoiseSigma)
                    : truth.fBody;
            ...
            const lastSampleT =
                sim.distHistory.length > 0
                    ? sim.distHistory[sim.distHistory.length - 1].t
                    : -Infinity;
            if (sim.simTime - lastSampleT >= 0.1) {
                sim.distHistory.push({
                    t: sim.simTime,
                    d: norm(sub(sim.truth.p, sim.exp.p)),
                });
            }
        };
    ...
    const noiseLabel = useMemo(
        () => `IMU noise σ: ${imuNoise.toFixed(2)}`,
        [imuNoise]
    );
    ...
    const truthCurrent =
        sim.truthTrail[sim.truthTrail.length - 1] ?? ([0, 0, 0] as Vec3);
    const expCurrent =
        sim.expTrail[sim.expTrail.length - 1] ?? ([0, 0, 0] as Vec3);
    ...
    return (
        <Paper
            elevation={3}
            sx={{ p: 3, borderRadius: 4, background: paperBg }}
        >
            <Stack
            ...
            <Paper
                variant="outlined"
                sx={{ mt: 2.5, p: 1.25, borderRadius: 4, overflow: "hidden" }}
            >
                <div
                    style={{
                        width: "100%",
                ...
                <Stack
                    direction="row"
                    spacing={2.5}
                    sx={{ mt: 1.25, px: 0.5, flexWrap: "wrap" }}
                >
                    {[
