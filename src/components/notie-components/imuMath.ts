...
export function addMat(A: Mat3, B: Mat3): Mat3 {
    return A.map(
        (row, i) => row.map((value, j) => value + B[i][j]) as Vec3
    ) as Mat3;
}
...
export function eulerBodyToWorld(
    roll: number,
    pitch: number,
    yaw: number
): Mat3 {
...
export function positionAt(
    ...
    return [2.2 * Math.sin(s), 1.6 * Math.sin(2 * s), 1.0 * Math.sin(3 * s)];
}
