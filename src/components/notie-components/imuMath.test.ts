...
describe("matMul associativity", () => {
    it("(A*B)*C = A*(B*C)", () => {
        ...
        const B: Mat3 = Ry(0.5);
        const C: Mat3 = Rz(0.7);
        expect(matClose(matMul(matMul(A, B), C), matMul(A, matMul(B, C)))).toBe(
            true
        );
    });
});
...
describe("velocityAt", () => {
    it("matches finite-difference of positionAt", () => {
        ...
        const h = 1e-5;
        const fdVel = scale(
            sub(positionAt(t + h), positionAt(t - h)),
            1 / (2 * h)
        );
...
describe("accelerationAt", () => {
    it("matches finite-difference of velocityAt", () => {
        ...
        const h = 1e-3;
        const fdAcc = scale(
            sub(velocityAt(t + h), velocityAt(t - h)),
            1 / (2 * h)
        );
...
describe("zero-noise integration", () => {
    ...
    function integrate(duration: number) {
        ...
        while (t < duration) {
            ...
            expR = matMul(expR, dR);
            expP = add(
                add(expP, scale(expV, DT)),
                scale(aWorld, 0.5 * DT * DT)
            );
...
describe("constant-accel mode: zero-noise integration is numerically exact", () => {
    ...
    it("position error < 1e-9 at dt=1/120 after 5s", () => {
        ...
        console.log(
            `  constant-accel dt=1/120  pos err = ${err.toExponential(3)} m`
        );
    ...
    it("position error < 1e-9 at dt=1/15 after 5s (large step, still exact)", () => {
        ...
        const err = norm(sub(expP, groundTruthAt(tFinal, "constant").p));
        console.log(
            `  constant-accel dt=1/15   pos err = ${err.toExponential(3)} m`
        );
        expect(err).toBeLessThan(1e-9);
    });
    ...
    it("position error < 1e-9 at dt=0.1 after 5s (very large step, still exact)", () => {
        ...
        const err = norm(sub(expP, groundTruthAt(tFinal, "constant").p));
        console.log(
            `  constant-accel dt=0.1    pos err = ${err.toExponential(3)} m`
        );
        expect(err).toBeLessThan(1e-9);
    });
    ...
    it("orientation error (Frobenius) < 1e-9 after 5s", () => {
        ...
        const err = frobeniusNorm(diff);
        console.log(
            `  constant-accel orientation err = ${err.toExponential(3)}`
        );
        expect(err).toBeLessThan(1e-9);
    });
